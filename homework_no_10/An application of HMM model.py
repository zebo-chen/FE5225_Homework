'''This is to train the HMM model to generate triangular wave'''
...
class HMM(object):
    def __init__(self,N,M):
        self.A = np.zeros((N,N))        # state transition probability matrix
        self.B = np.zeros((N,M))        # Observation probability matrix
        self.Pi = np.array([1.0/N]*N)   # Initial state probability matrix

        self.N = N                      # The number of possible states
        self.M = M                      # Possible number of observations
    ...
    def init(self):
        """
        Randomly generate A，B，Pi
        And make sure that each row adds up to 1
        """
        import random
        for i in range(self.N):
